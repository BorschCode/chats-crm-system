    ...
    <div class="bg-white shadow-sm rounded-lg p-6 mb-8">
        ...
        <div class="mt-4 flex items-center gap-4">
            ...
            <button wire:click="sortBy('title')"
                class="text-sm {{ $sortBy === 'title' ? 'text-blue-600 font-semibold' : 'text-gray-600' }} hover:text-blue-800">
                Name {{ $sortBy === 'title' ? ($sortDirection === 'asc' ? '↑' : '↓') : '' }}
            </button>
            <button wire:click="sortBy('price')"
                class="text-sm {{ $sortBy === 'price' ? 'text-blue-600 font-semibold' : 'text-gray-600' }} hover:text-blue-800">
                Price {{ $sortBy === 'price' ? ($sortDirection === 'asc' ? '↑' : '↓') : '' }}
            </button>
            <button wire:click="sortBy('created_at')"
                class="text-sm {{ $sortBy === 'created_at' ? 'text-blue-600 font-semibold' : 'text-gray-600' }} hover:text-blue-800">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach ($items as $item)
                    <a href="{{ route('item.show', $item->slug) }}" wire:navigate
                        class="group bg-white rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                        {{-- Item Image --}}
